...
from analytics.report_sections import (
    ...
    SPV_MODULES,
    QUIZ_MODULES,
    build_question_pdf_sections,
    build_spv_pdf_sections,
    build_quiz_pdf_sections,
)
...
class PdfSolutionProcessRequest(BaseModel):
    quiz_name: str
    records: list[dict[str, Any]] = Field(default_factory=list)
    question: str
    part_index: int = 1
...
# --- PDF export -----------------------------------------------------------
#
# report_sections.py's three build_*_pdf_sections() functions independently
# re-call the same chart-builder functions the screen routes above use
# (rather than reusing whatever's already been rendered) so the PDF is
# identical regardless of what's currently on screen — see that module's own
# docstrings. generate_pdf_report() then turns that sections list into raw
# PDF bytes via reportlab + kaleido (Plotly rasterization) + matplotlib
# (LaTeX/math rasterization, for table cells the on-screen route just sends
# as \\(...\\) text for the browser's own KaTeX to render — a PDF has no
# client-side renderer, so this is real rasterization, not just markup).

_REPORT_SECTIONS_BY_KIND = {
    "question": QUESTION_MODULES,
    "solutionprocess": SPV_MODULES,
    "quiz": QUIZ_MODULES,
}


@app.get("/report-sections/{kind}")
def report_sections_list(kind: str) -> dict[str, list[str]]:
    """The canonical section-name list for one report kind — drives the PHP
    side's "Generate PDF Report" checkboxes, fetched at render time so that
    UI can never drift from what the PDF route actually supports."""
    if kind not in _REPORT_SECTIONS_BY_KIND:
        raise HTTPException(status_code=404, detail=f"Unknown report-sections kind: {kind!r}")
    return {"sections": _REPORT_SECTIONS_BY_KIND[kind]}


@app.post("/pdf/question")
def pdf_question(req: PdfQuestionRequest) -> Response:
    """PDF for the Question Analysis sections of one quiz."""
    moodle_df = _records_to_moodle_df(req.records)
    if moodle_df.empty:
        raise HTTPException(status_code=400, detail="No records supplied.")
    ...
    selected = req.selected_sections if req.selected_sections else QUESTION_MODULES
    sections = build_question_pdf_sections(response_df, req.quiz_name, selected, req.colorblind_mode)
    pdf_bytes = generate_pdf_report(
        title=f"Question Analysis — {req.quiz_name}",
        subtitle="Generated by the Moodle STACK Interactive Analytics plugin",
        sections=sections,
    )
    return Response(content=pdf_bytes, media_type="application/pdf")


@app.post("/pdf/solution-process")
def pdf_solution_process(req: PdfSolutionProcessRequest) -> Response:
    """PDF for the Solution Process Visualization sections of one (quiz,
    question, part)."""
    moodle_df = _records_to_moodle_df(req.records)
    if moodle_df.empty:
        raise HTTPException(status_code=400, detail="No records supplied.")
    ...
    selected = req.selected_sections if req.selected_sections else SPV_MODULES
    sections = build_spv_pdf_sections(
        response_df, req.quiz_name, req.question, req.part_index, selected, req.colorblind_mode,
    )
    pdf_bytes = generate_pdf_report(
        title=f"Solution Process Visualization — {req.quiz_name}",
        subtitle=f"{req.question}, part {req.part_index} — Generated by the Moodle STACK Interactive Analytics plugin",
        sections=sections,
    )
    return Response(content=pdf_bytes, media_type="application/pdf")
...
@app.post("/pdf/quiz")
def pdf_quiz(req: PdfQuizRequest) -> Response:
    """PDF for the cross-quiz Quiz Analysis sections, across every quiz in
    `req.quizzes`."""
    frames = []
    for quiz_name, records in req.quizzes.items():
        moodle_df = _records_to_moodle_df(records)
    ...
    combined = pd.concat(frames, ignore_index=True)
    quiz_names = list(req.quizzes.keys())

    selected = req.selected_sections if req.selected_sections else QUIZ_MODULES
    sections = build_quiz_pdf_sections(combined, quiz_names, selected, req.colorblind_mode)
    pdf_bytes = generate_pdf_report(
        title=f"Quiz Analysis — {req.course_name}",
        subtitle="Generated by the Moodle STACK Interactive Analytics plugin",
        sections=sections,
    )
    return Response(content=pdf_bytes, media_type="application/pdf")
