...
| Math in the PDF reads as plain text/Unicode (`√(3)`, `9·i·sin(...)`) rather than typeset symbols | Expected, current behavior: the PDF has no LaTeX typesetting engine, so it renders math as readable plain text instead of an image — the on-screen page still typesets it properly via KaTeX |
