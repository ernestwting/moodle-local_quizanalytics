...
if ($result === false) {
    ...
    // Must be normalized exactly like dispatch_for_course() normalizes its
    // own $quizids before building the customdata it dispatches with —
    // get_queued_age_seconds()/is_stuck() below match by exact encoded
    // customdata, and $stackquizzes here is keyed in chronological quiz
    // order (get_course_stack_quizzes()), not sorted-ascending-by-id, so
    // passing array_keys($stackquizzes) through unnormalized would never
    // match the task this page itself just dispatched.
    $taskcustomdata = [
    ...
    // unstarted/unfinished longer than a healthy cron cycle ever should
    // most likely means cron isn't running on this site at all (see
    // warm_single_view_adhoc_task::INLINE_FALLBACK_SECONDS) — compute it
    // here instead of leaving the visitor stuck on this notice forever
    // every time they revisit. This plugin's Quiz Analytics must work
    // whether or not the site this runs on has cron configured.
