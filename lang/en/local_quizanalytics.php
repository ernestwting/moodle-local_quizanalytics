...
$string['generatingstale'] = 'This has been queued for {$a} without finishing. That is longer than any real background compute has taken on this site, so this most likely means Moodle\'s cron isn\'t running, the background task crashed or ran out of memory, or cron is stuck behind a different, unrelated task that keeps failing and retrying. Ask your Moodle administrator to check Site administration → Server → Scheduled tasks (is cron running at all?) and Site administration → Server → Tasks → Task logs — for this task having failed, but also for a completely different task failing over and over, which can hog every cron run and starve this one even though nothing here is actually broken — rather than waiting longer.';
...
$string['parallelworkers'] = 'Cache-warming parallel workers (currently disabled — see description)';
$string['parallelworkers_desc'] = '<strong>Temporarily has no effect as of 3.0.2.</strong> Forking was found to be able to crash the entire Moodle cron.php process outright (not just this task) on a large enough course — a corrupted-database-connection issue in how a forked worker pool reconnects after fork(), which collides with a lock Moodle core itself holds across this task\'s execute() call. Confirmed directly: a stuck "generating in the background" page turned out to be caused by this crashing cron before it ever reached the queued background task, silently, on every single cron run, for as long as the triggering course stayed that large. Forking is disabled site-wide (parallel_course_fetcher::fetch() always takes its sequential fallback now) until this can be fixed properly — this setting is left in place, but changing it does nothing right now. The "Warm STACK q-type Analytics result caches" scheduled task still runs and still warms every course\'s cache, just always one quiz at a time.';
$string['parallelworkermemory'] = 'Cache-warming worker memory limit (MB)';
$string['parallelworkermemory_desc'] = 'The PHP memory limit (in MB) the cache-warming fetch is allowed. The fetch streams one quiz\'s records to disk at a time rather than accumulating its whole share in memory, but a single very large quiz\'s own attempt data still has to fit in memory on its own. Measured directly, one 2,136-attempt quiz alone peaked at just under 1GB, which is why the default here is higher than that. (Formerly also sized together with "Cache-warming parallel workers" for the multi-worker case — that pairing does not currently apply; see that setting\'s own description.)';
