...
$string['generatingstale'] = 'Esto ha estado en cola durante {$a} sin terminar. Eso es más tiempo del que ha tardado cualquier cálculo en segundo plano real en este sitio, por lo que lo más probable es que el cron de Moodle no se esté ejecutando, que la tarea en segundo plano haya fallado o se haya quedado sin memoria, o que el cron esté atascado detrás de otra tarea, no relacionada, que falla y se reintenta continuamente. Pida a su administrador de Moodle que revise Administración del sitio → Servidor → Tareas programadas (¿se está ejecutando el cron?) y Administración del sitio → Servidor → Tareas → Registros de tareas — tanto si esta tarea ha fallado, como si una tarea completamente distinta está fallando repetidamente, lo que puede acaparar cada ejecución del cron y dejar esta tarea sin turno aunque aquí no haya nada realmente roto — en lugar de seguir esperando.';
